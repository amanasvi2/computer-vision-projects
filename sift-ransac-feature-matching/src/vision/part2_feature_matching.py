...
def match_features_ratio_test(
    ...
    if n2 < 2:
        ...
        matches = np.stack((np.arange(n1), j), axis=1).astype(int)
        confidences = np.ones((n1,), dtype=np.float32)
        return matches, confidences
    two_best = np.argpartition(dists, 1, axis=1)[:, :2].astype(int)
    row_idx = np.arange(n1)
    d0 = dists[row_idx, two_best[:, 0]]
    d1 = dists[row_idx, two_best[:, 1]]
    swap = d0 > d1
    best_idx = np.where(swap, two_best[:, 1], two_best[:, 0]).astype(int)
    second_best_idx = np.where(swap, two_best[:, 0], two_best[:, 1]).astype(int)
    best_dist = dists[row_idx, best_idx]
    second_best_dist = dists[row_idx, second_best_idx]
    eps = 1e-12
    ratio = best_dist / (second_best_dist + eps)
    ratio_threshold = 0.8
    keep = ratio < ratio_threshold
    matches = np.stack((row_idx[keep], best_idx[keep]), axis=1).astype(int)
    confidences = (1.0 / (ratio[keep] + eps)).astype(np.float32)
