...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    kx = torch.from_numpy(SOBEL_X_KERNEL).unsqueeze(0).unsqueeze(0)
    ky = torch.from_numpy(SOBEL_Y_KERNEL).unsqueeze(0).unsqueeze(0)
    conv_x = nn.Conv2d(
        in_channels=1,
        out_channels=1,
        kernel_size=3,
    ...
    conv_x.weight = nn.Parameter(kx)
    conv_y.weight = nn.Parameter(ky)

    Ix = conv_x(img_t).squeeze(0).squeeze(0).detach().numpy()
    Iy = conv_y(img_t).squeeze(0).squeeze(0).detach().numpy()
...
def get_harris_interest_points(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    R = compute_harris_response_map(image_bw, ksize=7, sigma=5, alpha=0.05)
