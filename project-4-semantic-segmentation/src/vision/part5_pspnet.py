...
class PSPNet(nn.Module):
    ...
    def __init__(
        ...
        resnet_fea_dim = 2048
        reduction_dim = resnet_fea_dim // len(bins)
        if use_ppm:
            self.ppm = PPM(bins=list(bins), in_dim=resnet_fea_dim, reduction_dim=reduction_dim)
            fea_dim = resnet_fea_dim + reduction_dim * len(bins)
        else:
            self.ppm = None
            fea_dim = resnet_fea_dim
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for name, module in self.layer3.named_modules():
            if name.endswith('conv2'):
                ...
                module.dilation = (2, 2)
                module.padding = (2, 2)
            elif name.endswith('downsample.0'):
                module.stride = (1, 1) 
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        cls = nn.Sequential(
            nn.Conv2d(in_channels=in_feats, out_channels=out_feats, kernel_size=3, padding=1),
    ...
    def forward(
        ...
        out_H = math.ceil(H / 8 * self.zoom_factor)
        out_W = math.ceil(W / 8 * self.zoom_factor)
        x = self.layer0(x)
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)
        ...
        if self.use_ppm and self.ppm is not None:
            x = self.ppm(x)
        x = self.cls(x)
        logits = F.interpolate(x, size=(out_H, out_W), mode='bilinear', align_corners=True)
        yhat = torch.argmax(logits, dim=1)
        if self.training and y is not None:
            main_loss = self.criterion(logits, y)
            aux_loss = self.criterion(F.interpolate(aux_out, size=(out_H, out_W), mode='bilinear', align_corners=True), y)
        else:
            main_loss = None
            aux_loss = None
