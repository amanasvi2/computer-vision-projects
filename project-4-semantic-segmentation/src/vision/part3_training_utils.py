...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == 'SimpleSegmentationNet':
        ...
        optimizer = torch.optim.SGD(resnet_params + head_params)
    else:
        model = PSPNet(
            layers=getattr(args, "layers", 50),
            bins=getattr(args, "bins", (1, 2, 3, 6)),
            dropout=getattr(args, "dropout", 0.1),
            zoom_factor=getattr(args, "zoom_factor", 8),
            use_ppm=getattr(args, "use_ppm", True),
            num_classes=args.classes, 
            pretrained=getattr(args, "pretrained", True), 
            criterion=nn.CrossEntropyLoss(ignore_index=args.ignore_label))
        resnet_params = [
            {"params": model.layer0.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay, "lr": args.base_lr},
            {"params": model.layer1.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay, "lr": args.base_lr},
            {"params": model.layer2.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay, "lr": args.base_lr},
            {"params": model.layer3.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay, "lr": args.base_lr},
            {"params": model.layer4.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay, "lr": args.base_lr}
        ]
        head_params = [
