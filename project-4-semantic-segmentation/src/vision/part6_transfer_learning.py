...
def model_and_optimizer(args, model) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    num_kitti_classes = 2
    fea_dim = model.cls[0].in_channels
    aux_inc = model.aux[0].in_channels
    model.cls = nn.Sequential(
        nn.Conv2d(in_channels=fea_dim, out_channels=fea_dim, kernel_size=3, padding=1, bias=False),
        nn.BatchNorm2d(fea_dim),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=model.dropout),
        nn.Conv2d(in_channels=fea_dim, out_channels=num_kitti_classes, kernel_size=1)
    )
    model.aux = nn.Sequential(
        nn.Conv2d(in_channels=aux_inc, out_channels=aux_inc, kernel_size=3, padding=1, bias=False),
        nn.BatchNorm2d(aux_inc),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=model.dropout),
        nn.Conv2d(in_channels=aux_inc, out_channels=num_kitti_classes, kernel_size=1)
    )
    ...
    optimizer = torch.optim.SGD([
        ...
        { "params": model.ppm.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay},
        { "params": model.cls.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay},
        { "params": model.aux.parameters(), "lr": args.base_lr, "momentum": args.momentum, "weight_decay": args.weight_decay}
    ])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
