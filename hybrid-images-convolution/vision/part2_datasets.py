...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    for filename in sorted(os.listdir(path)):
        ...
        if os.path.isfile(filepath) and filename.endswith('.bmp'):
            base_name = filename[:-4]
            if base_name[-1] == 'a' or '_a_' in filename or filename[1] == 'a':
                images_a.append(filepath)
            elif base_name[-1] == 'b' or '_b_' in filename or filename[1] == 'b':
                images_b.append(filepath)
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    with open(path, "r") as f:
        for line in f:
            cutoff_freq.append(int(line.strip()))
