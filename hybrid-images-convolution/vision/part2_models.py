...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        k = kernel_2d.shape[0]
        kernel = np.tile(kernel_2d.reshape(1, 1, k, k), (self.n_channels, 1, 1, 1))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        filtered_image = F.conv2d(
            x, kernel, padding=pad, groups=self.n_channels
        )
    ...
    def forward(
        ...
        if cutoff_frequency.numel() == 1:
            cf = int(cutoff_frequency.item())
        else:
            cf = int(cutoff_frequency[0].item())
